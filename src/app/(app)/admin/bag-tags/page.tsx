import { createClient } from "@/lib/supabase/server";
import { redirect } from "next/navigation";
import Link from "next/link";
import { ChevronLeft } from "lucide-react";
import { AssignTagForm } from "@/components/admin/assign-tag-form";
import { CreateTagsForm } from "@/components/admin/create-tags-form";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Admin — Bag Tags" };

export default async function AdminBagTagsPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/login");

  const { data: profile } = await supabase
    .from("profiles")
    .select("role")
    .eq("id", user.id)
    .single();
  if ((profile as any)?.role !== "admin") redirect("/dashboard");

  const currentSeason = new Date().getFullYear().toString();

  const [{ data: tags, error: tagsError }, { data: members }] =
    await Promise.all([
      supabase
        .from("bag_tags")
        .select(
          "*, holder:profiles!bag_tags_holder_id_fkey(id, full_name, username)",
        )
        .eq("is_active", true)
        .eq("season", currentSeason)
        .order("tag_number", { ascending: true }),
      supabase
        .from("profiles")
        .select("id, full_name, username, current_tag_id")
        .eq("is_active", true)
        //.eq("season", currentSeason)
        .order("full_name", { ascending: true }),
    ]);
  console.log("DEBUG tags query result:", JSON.stringify(tags, null, 2));
  console.log("DEBUG tags error:", JSON.stringify(tagsError, null, 2));

  const allTags = (tags as any[]) ?? [];
  const allMembers = (members as any[]) ?? [];
  const claimed = allTags.filter((t) => t.holder_id);
  const unclaimed = allTags.filter((t) => !t.holder_id);
  const maxTag = allTags.reduce(
    (max: number, t: any) => Math.max(max, t.tag_number),
    0,
  );

  // Members without a tag
  const membersWithoutTag = allMembers.filter((m) => {
    if (!m.current_tag_id) return true;
    // Allow reassignment if their current tag is from a previous season
    const theirTag = allTags.find((t) => t.id === m.current_tag_id);
    return !theirTag; // not in this season's tags
  });

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin" className="p-2 hover:bg-gray-100 rounded-lg">
          <ChevronLeft size={20} className="text-gray-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Bag Tags</h1>
          <p className="text-gray-500 text-sm">
            {claimed.length} assigned · {unclaimed.length} available ·{" "}
            {currentSeason} Season
          </p>
        </div>
      </div>

      {/* Create more tags */}
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <h2 className="font-semibold text-gray-900 mb-4">Create New Tags</h2>
        <CreateTagsForm currentMax={maxTag} />
      </div>

      {/* Assign tag to member */}
      {unclaimed.length > 0 && allMembers.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-900 mb-1">
            Assign Tag to Member
          </h2>
          <p className="text-xs text-gray-500 mb-4">
            Assign an available tag number to a member who has paid for their
            tag.
          </p>
          <AssignTagForm unclaimedTags={unclaimed} members={allMembers} />
        </div>
      )}

      {/* Current assignments */}
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
            All Tags — {currentSeason}
          </p>
          <p className="text-xs text-gray-400">
            {claimed.length} / {allTags.length} assigned
          </p>
        </div>
        <div className="divide-y divide-gray-100">
          {allTags.map((tag: any) => (
            <div key={tag.id} className="flex items-center gap-3 px-4 py-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 ${
                  tag.holder_id
                    ? "bg-green-100 text-green-700"
                    : "bg-gray-100 text-gray-400"
                }`}
              >
                #{tag.tag_number}
              </div>
              <div className="flex-1 min-w-0">
                {tag.holder_id ? (
                  <p className="text-sm font-semibold text-gray-900 truncate">
                    {tag.holder?.full_name ?? tag.holder?.username ?? "Unknown"}
                  </p>
                ) : (
                  <p className="text-sm text-gray-400 italic">Unassigned</p>
                )}
              </div>
              {tag.holder_id && (
                <ReleaseTagForm tagId={tag.id} holderId={tag.holder_id} />
              )}
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}

function ReleaseTagForm({
  tagId,
  holderId,
}: {
  tagId: string;
  holderId: string;
}) {
  return (
    <form
      action={async () => {
        "use server";
        const { createClient } = await import("@/lib/supabase/server");
        const supabase = await createClient();
        await (supabase as any)
          .from("profiles")
          .update({ current_tag_id: null })
          .eq("id", holderId);
        await (supabase as any)
          .from("bag_tags")
          .update({ holder_id: null })
          .eq("id", tagId);
      }}
    >
      <button
        type="submit"
        className="text-xs text-red-500 hover:text-red-600 font-medium px-2 py-1 hover:bg-red-50 rounded-lg transition-colors"
      >
        Release
      </button>
    </form>
  );
}
